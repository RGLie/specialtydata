...
const defaultForm: RoasteryForm = {
  name: "",
  description: "",
  website: "",
  location: "",
...
export default function NewRoasteryPage() {
  const [form, setForm] = useState<RoasteryForm>(defaultForm);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const roasteryData = {
        originalId: form.name.toLowerCase().replace(/\s+/g, '-'),
        ...form,
        createdAt: new Date(),
        updatedAt: new Date()
      };

      await firestoreService.add(COLLECTIONS.ROASTERIES, roasteryData);
      router.push('/admin/roasteries');
    } catch (error) {
      console.error('로스터리 추가 실패:', error);
      alert('로스터리 추가 중 오류가 발생했습니다.');
    } finally {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-bold text-gray-900">새 로스터리 추가</h1>
            </div>
          </div>
        </div>
      </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={form.founded}
                  onChange={(e) => setForm(prev => ({ ...prev, founded: parseInt(e.target.value) || new Date().getFullYear() }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-coffee-brown focus:border-coffee-brown"
                  min="1800"
            ...
            <div className="space-y-4">
              <label className="flex items-center space-x-3">
                <input
                  ...
                  onChange={(e) => setForm(prev => ({ ...prev, isUnspecialtyPartner: e.target.checked }))}
